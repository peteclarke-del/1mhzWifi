...
run_client() {
    ...
    timeout 30s docker run --rm --ipc=host \
        ...
        ./elkulator -disc "/discs/$disc" -autoboot >"$log" 2>&1 || {
            cat "$log" >&2
...
run_client term term-smoke.ssd
run_client ssh ssh-smoke.ssd

term_trace=$trace_dir/term.trace
ssh_trace=$trace_dir/ssh.trace
grep -q $'^OPEN\t0\tTELNET://fixture/' "$term_trace"
grep -q $'^CLOSE\t0\tTELNET://fixture/' "$term_trace"
grep -q $'^SSH_OPEN\t0\tTCP://fixture:22/' "$ssh_trace"
grep -q $'^SSH_USER\t0\ttest' "$ssh_trace"
grep -q $'^CLOSE\t0\tTCP://fixture:22/' "$ssh_trace"

term_read=$(awk -F '\t' '$1 == "READ" { printf "%s", $3 }' "$term_trace")
ssh_read=$(awk -F '\t' '$1 == "READ" { printf "%s", $3 }' "$ssh_trace")

[[ "$term_read" == *"5069314d487a206d61696c626f78204f4b"* ]]
[[ "$ssh_read" == *"5069314d487a20535348207368656c6c204f4b"* ]]

echo "Full Elkulator TERM/SSH Pi1MHz mailbox smoke tests: OK"
