...
#define ELKWIFI_UEF_OP_REPUBLISH 7u
...
static uint8_t uef_incremental_command(uint32_t cp)
{
   ...
   switch (operation) {
      ...
      case ELKWIFI_UEF_OP_REPUBLISH:
         /* The host's service reply buffer is the first bytes of JIM page 0,
          * which is also where the published window starts, so any command
          * that copies a reply while a stream is open overwrites the start of
          * that window. Lay the same bytes down again without moving the
          * cursor or the generation: the host has not consumed anything, it
          * has only had part of the window trodden on. */
         if (!uef_stream_ready
             || (token != 0u && token != uef_stream_token))
            return ELKWIFI_ERR_PARAM;
         uef_stream_republish_window();
