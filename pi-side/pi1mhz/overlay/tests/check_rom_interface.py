...
# Installed into a Pi1MHz checkout by the 1MHzWifi installer, next to the
# headers, and given the host ROM's service_driver.asm as its argument. The
# ROM is not in that tree - 1MHzWifi keeps it - so the path is passed in
# rather than derived.
HERE = Path(__file__).resolve().parent
ROOT = HERE.parents[2]                      # repo root
ROM = Path(sys.argv[1]) if len(sys.argv) > 1 \
    else ROOT / "beeb/1mhz-wifi/src/service_driver.asm"
