...
| 18 | Return interface configuration |
...
and `&FCFF`. Callers should reselect the upper bytes before consuming a
returned buffer because MMFS and other expansions share the aperture.
...
The current test ROM reports `1MHzWifi 0.1.13 (C) 2026 Peter Clarke`, followed
...
kernel revision, for example `Pi1MHz ElkWiFi 0.1.13, kernel
V1.30-80-g8468a38-dirty.<hash>`.
