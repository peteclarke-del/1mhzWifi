...
| `*WGET -U <url>` | Downloads a UEF image to JIM `&010000-&01FFFF` | Implemented; hardware validation pending |
...
Every driver entry selects JIM address `00:00:page` through `&FCFD`, `&FCFE`
and `&FCFF`. The page selector is write-only on AP5/Pi1MHz, so the ROM keeps a
...
The current test ROM reports `1MHzWifi 0.1.17 (C) 2026 Peter Clarke`, followed
...
kernel revision, for example `Pi1MHz ElkWiFi 0.1.17, kernel
V1.30-80-g8468a38-dirty.5ca5cd71`.

### `*UEF LOAD <filename>`

Opens a UEF image through the currently selected MOS filing system, streams it
into Pi1MHz JIM window 1, writes the same length trailer as `*WGET -U`, selects
...
importer reselects its full JIM address after every OSBGET because the source
...
failure, or overflow. The launch remains entirely on the I/O processor and
does not introduce a Tube transfer path.
