...
fits the bounded 239-byte application mailbox; `*ONLINE` reports link readiness.
Function 9
returns `OK`, CR, LF, NUL with response length `&0004`. Callers should reselect
...
equal-length call to a Pi1MHz JIM address helper. Custom menu payloads that do not
...
Whole-file loads retain all four OSFILE address bytes. Pi1MHz supplies the UEF
over the 1MHz bus in every configuration. A normal host address is written to
Electron I/O-processor memory. If OSBYTE `&EA` reports an active Tube and the
caller supplies a standard `&FFFFxxxx` parasite address, WiCFS claims its
filing-system Tube channel, sends the bytes through Tube R3, then releases the
claim. The Tube is never the Pi transport. OSFILE returns the load, execution,
length and attribute fields required by callers. Catalogue,
...
The current test ROM reports `1MHzWifi 0.1.25 (C) 2026 Peter Clarke`, followed
...
kernel revision, for example `Pi1MHz ElkWiFi 0.1.25, kernel
V1.30-80-g8468a38-dirty.29afd03a`.

### `*UEF LOAD <filename>`

Opens a UEF image through the currently selected MOS filing system, streams it
into the AP5-visible Pi1MHz JIM window, writes the same length trailer as `*WGET -U`, selects
the tape filing system, installs WiCFS, rewinds, and executes `CHAIN ""`. The
...
multi-file applications. Release 0.1.10 checks the cassette block's declared
length before fetching data, so a final zero-byte file such as Desk Diary's
