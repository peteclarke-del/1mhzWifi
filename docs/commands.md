...
| `*VERSION` | Prints the `1MHzWiFi` ROM revision and Pi service response |

The current test ROM identifies itself as `1MHzWiFi 0.1.0`. This first line is
generated by the ROM and can be used to confirm the exact host image before
reporting hardware results. The following line identifies the Pi-side service.
